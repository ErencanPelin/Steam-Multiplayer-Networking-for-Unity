...
namespace Network.Framework
{
    ...
    /// </summary>
    public class NW_NetworkObjectPool : NetworkBehaviour
    {
        private static NW_NetworkObjectPool _instance;

        public static NW_NetworkObjectPool Singleton { get { return _instance; } }
        ...
        public void OnValidate()
        {
            for (var i = 0; i < PooledPrefabsList.Count; i++)
            {
                ...
                if (prefab != null)
                {
                    Assert.IsNotNull(prefab.GetComponent<NetworkObject>(), $"{nameof(NW_NetworkObjectPool)}: Pooled prefab \"{prefab.name}\" at index {i} has no {nameof(NetworkObject)} component.");
                }
            }
        }
    ...
    {
        GameObject m_Prefab;
        NW_NetworkObjectPool m_Pool;

        public PooledPrefabInstanceHandler(GameObject prefab, NW_NetworkObjectPool pool)
        {
            m_Prefab = prefab;
            m_Pool = pool;
        }
